...
LiquidCrystal lcd(3, 4, 5, 8, 11, 12);
...
static const u1_t PROGMEM DEVEUI[8]={ 0x98, 0x60, 0x94, 0xB8, 0xA1, 0xB5, 0xAF, 0x7C };
...
static const u1_t PROGMEM APPKEY[16] = { 0x6E, 0xA5, 0xD8, 0x03, 0x4C, 0xB7, 0xBB, 0x93, 0x65, 0xBB, 0xDE, 0x95, 0x1D, 0x2F, 0x01, 0x3B };
void os_getDevKey (u1_t* buf) {  memcpy_P(buf, APPKEY, 16);}

static uint8_t mydata[] = "Hello, world!";
static osjob_t sendjob;

// Schedule TX every this many seconds (might becom longer due to duty
// cycle limitations).
...
void loop() {
    ...
    if ( millis() > t_start + t_wait) {
      ...
      if ( cal_cnt == 0 ) {
        ...
        dist = lidarLite.distance(false); // Withouct bias correction
      }
